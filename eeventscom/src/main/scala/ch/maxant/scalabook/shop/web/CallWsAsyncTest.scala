...
@WebServlet(urlPatterns = Array("/CallWsAsyncTest"), asyncSupported = true)
class CallWsAsyncTest extends HttpServlet {
       
	override def doGet(request: HttpServletRequest, response: HttpServletResponse) = {
		val aCtx = request.startAsync(request, response)
		...
		val r = new Runnable(){
			def run() {
			    ...
			    println("got " + result)
			    aCtx.getResponse().getOutputStream().print("got result: " + result)
